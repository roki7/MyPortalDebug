...
import { 
  ...
  FormControl, InputLabel, Select, MenuItem, Box, Divider, Chip, 
...
export default function JobEditDialog({ open, onClose, job, members, onSave }) {
  ...
  const [editJob, setEditJob] = useState({ 
    ...
    memberId: 'me', closingDay: 99, payTiming: 'next', payDay: 25 
  });
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
      ...
      <DialogContent sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <TextField 
          label="仕事名" fullWidth value={editJob.name || ''} 
        ...
        <TextField 
          label="休憩 (分)" type="number" fullWidth 
          ...
          <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5 }}>
            {WEEKDAYS.map((day, i) => (
              <Chip 
                ...
                onClick={() => {
                  const newDays = editJob.days?.includes(i) ? editJob.days.filter(d => d !== i) : [...(editJob.days || []), i];
