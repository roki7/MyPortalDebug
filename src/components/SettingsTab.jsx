    ...
    <Box>
      ...
      <Dialog open={openJobDialog} onClose={() => setOpenJobDialog(false)}>
        ...
        <DialogContent>
          ...
          <Typography variant="subtitle2" sx={{mt:1, color:'primary.main'}}>詳細設定</Typography>
          <Grid container spacing={2} sx={{mb:2}}>
            <Grid item xs={6}>
                <TextField label="開始時間" type="time" size="small" fullWidth InputLabelProps={{shrink:true}} value={editingJob.defaultStart || '09:00'} onChange={(e)=>setEditingJob({...editingJob, defaultStart:e.target.value})} />
            </Grid>
            <Grid item xs={6}>
